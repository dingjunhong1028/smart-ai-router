...
import { TEN_WINGS_APOSTLES } from './ten-wings';
import { APOSTLE_RUNE_MAP } from './rune-registry';

/**
 * 初始化右翼使徒代理小組 (ADK)
 */
export function initApostleAgents() {
  TEN_WINGS_APOSTLES.forEach(apostle => {
    ...
    apostle.agent = AdkSquadFactory.createAgent({
      ...
      instruction: `你代表 ADK 十翼使徒中的「${apostle.name}」(${apostle.nameEn})。
      你的職責是：${apostle.role}
